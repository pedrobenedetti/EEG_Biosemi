...
# Image available on https://drive.google.com/file/d/1khTMM9Fs5iIt-V4cTV5PTUFyKDczdU0g/view?usp=share_link
newsplane = pygame.image.load(path + '/newsplane.png')
# Image available on https://drive.google.com/file/d/1RboI8fX5lwLC24OwUrFYIdizChEHX6wy/view?usp=share_link
...
soundObj = pygame.mixer.Sound(path + '/bell.wav')

pag_count = -1  # Indicates in wich page of the protocol we are located
...
while True:  # permanently running
    if pag_count == -1:  # PARTICIPANT CODE INPUT
        window.fill((238, 238, 238))
        text_surface = title_font.render(
            "Codigo de participante: ", True, (234, 64, 142))
        ...
        text_surface3 = input_font.render(
            "Presioná 'Enter' para avanzar.", True, (74, 64, 103))
    ...
    if pag_count == 1:  # PART 1 TITLE (AUT)
        ...
        text_surface8 = input_font.render(
            "Presioná 'Enter' para avanzar.", True, (74, 64, 103))
    ...
    if pag_count == 2:  # INSTRUCTIONS FOR AUT
        ...
        text_surface = title_font.render(
            "INSTRUCCIONES:", True, (234, 64, 142))
        window.blit(text_surface, (X/2-text_surface.get_width()/2, 50))

        text_surface2 = input_font.render(
            "El objetivo es escribir todos los usos alternativos que se te ocurran para un objeto.", True, (74, 64, 103))
        window.blit(text_surface2, (X/2-text_surface2.get_width()/2, Y/2-150))

        text_surface3 = input_font.render(
            "Después de iniciar, escribí tus propuestas y presioná 'Enter' al terminar cada una.", True, (74, 64, 103))
        window.blit(text_surface3, (X/2-text_surface3.get_width()/2, Y/2-125))

        text_surface4 = input_font.render(
            "Usá tu imaginación para generar tus propuestas. No hay límites.", True, (74, 64, 103))
        window.blit(text_surface4, (X/2-text_surface4.get_width()/2, Y/2-75))

        text_surface5 = input_font.render(
            "Tu única limitación es el tiempo, vas a disponer de 1 minuto para pensar y 4 para escribirlos.", True, (74, 64, 103))
        window.blit(text_surface5, (X/2-text_surface5.get_width()/2, Y/2-50))

        text_surface6 = input_font.render(
            "Te damos un ejemplo: ¿Que usos le darías a un papel de diario?", True, (74, 64, 103))
        window.blit(text_surface6, (X/2-text_surface6.get_width()/2, Y/2+0))

        text_surface7 = input_font.render(
            "Una respuesta podría ser 'Hacer un avión de papel'", True, (74, 64, 103))
    ...
    if pag_count == 3:  # Thinking AUT
        ...
        time_now = datetime.datetime.now()
        text_surface = title_font.render(
            "Pensá: ¿Qué usos alternativos le darías a un clip?", True, (234, 64, 142))
        ...
        text_surface3 = input_font.render(
            "En breve vas a poder escribirlos.", True, (74, 64, 103))
    ...
    if pag_count == 4:  # AUT
        ...
        if started_AUT == 0:
            if (time_now-time_start).total_seconds() < 1:
                ...
                window.blit(
                    text_surface, (X/2-text_surface.get_width()/2, Y/2))
            ...
            text_surface = title_font.render(
                "¿Qué usos alternativos le darías a un clip?", True, (234, 64, 142))
            window.blit(text_surface, (X/2-text_surface.get_width()/2, 5))
            if len(input_text) > 0:
                for x in input_text:
                    c = c+1
                    text_surface = input_font.render(
                        (str(c)+") "+x), True, (74, 64, 103))
                    window.blit(text_surface, (input_rect.x +
                                5, input_rect.y + c * 30+15))
                    input_rect.w = max(100, text_surface.get_width()+10)
                    input_rect.h = text_surface.get_height()*N_uses+10
            text_surface2 = input_font.render(
                (str(N_uses+1)+") " + current_use), True, (74, 64, 103))
            window.blit(text_surface2, (input_rect.x + 5,
                        input_rect.y + (N_uses+1) * 30+15))

            segs = (time_now-time_start).total_seconds()
            mins = math.modf(3-(segs // 60))
            segs_rest = math.modf(60-(segs % 60))
            text_surface3 = time_font.render(
                str(int(mins[1]))+":"+str(int(segs_rest[1])), True, (234, 64, 142))
            window.blit(text_surface3, (X-clip.get_width() /
                        2-text_surface3.get_width()/2, 10))
            window.blit(clip, (X-clip.get_width(), Y/2-clip.get_height()/2))
    ...
    if pag_count == 5:  # TILE PART 2 (REY FIGURE TEST)
        ...
        text_surface = input_font.render(
            "Se acabó el tiempo. Pasamos a la siguiente parte:", True, (74, 64, 103))
        window.blit(text_surface, (X/2-text_surface.get_width()/2, 50))
    ...
    if pag_count == 6:  # INSTRUCTIONS REY FIGURE TEST
        ...
        text_surface2 = input_font.render(
            "Por último, tenés que dibujar de memoria la misma figura que hace dos días.", True, (74, 64, 103))
        window.blit(text_surface2, (X/2-text_surface2.get_width()/2, Y/2))

        text_surface3 = input_font.render(
            "Tenés un tiempo de 1 minuto para pensar y 3 minutos para dibujar.", True, (74, 64, 103))
        window.blit(text_surface3, (X/2-text_surface3.get_width()/2, Y/2+50))

        text_surface4 = input_font.render(
            "Presioná 'Enter' para comenzar.", True, (74, 64, 103))
    ...
    if pag_count == 7:  # Thinking REY
        ...
        time_now = datetime.datetime.now()
        text_surface = title_font.render(
            "Recordá la figura que dibujaste hace dos días.", True, (234, 64, 142))
        window.blit(text_surface, (X/2-text_surface.get_width()/2, 5))
        ...
        text_surface3 = input_font.render(
            "En breve vas a poder comenzar a dibujarla.", True, (74, 64, 103))
        window.blit(text_surface3, (X/2-text_surface3.get_width()/2, Y-50))
    ...
    if pag_count == 9:  # THANKS
        window.fill((238, 238, 238))
        text_surface = title_font.render(
            "Muchas gracias por participar!", True, (234, 64, 142))
        window.blit(text_surface, (X/2-text_surface.get_width()/2, Y/2))

        text_surface2 = title_font.render(
            "Solo te pedimos que prestes atención durante", True, (234, 64, 142))
        window.blit(text_surface2, (X/2-text_surface2.get_width() /
                    2, Y/2+text_surface2.get_height()))

        text_surface3 = title_font.render(
            "un minuto a la cruz de la siguiente pantalla.", True, (234, 64, 142))
        window.blit(text_surface3, (X/2-text_surface3.get_width() /
                    2, Y/2+text_surface2.get_height()*2))
...
#############################################################################################
    for event in pygame.event.get():
        if event.type == pygame.KEYDOWN:
            ...
            # 'Enter' Button. Advance in pages and saves uses.
